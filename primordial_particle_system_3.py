...
spread = 1
#np.random.seed(42)
pos = np.stack([np.random.uniform(0+z*(1-spread)/2,z*(1-(1-spread)/2),n_particles),np.random.uniform(0+z*(1-spread)/2,z*(1-(1-spread)/2),n_particles)]).T
...
def move(pos,orient):
    ...
    #orient[pos < 0] *= -1
    #orient[pos > z] *= -1
    #pos[pos < 0] -= v*orient[pos < 0]
    #pos[pos > z] -= v*orient[pos > z]
    pos[pos < 0] += 2*v
    pos[pos > z] -= 2*v
    return pos
...
while running:
    ...
    if not pause:
        ...
        grid_contents = np.zeros((int(znew/neighbor_radius)+1,int(znew/neighbor_radius)+1,n_particles),dtype=bool)
        ...
        grid_contents[isparticle[:,0],isparticle[:,1],isparticle[:,2]] = True
        ...
        for i in range(grid_contents.shape[0]):
            for j in range(grid_contents.shape[1]):
                particles = np.argwhere(grid_contents[i,j]).flatten()
                if len(particles) == 0:
                    continue
        ...
        rotationpart[rotationpart < 0] = -1
        t4 = datetime.now()
        col = 255*cm(np.minimum(np.ones_like(np.sum(neighbors,axis=1)),np.log10(np.sum(neighbors,axis=1)+ 1e-6)/np.log10(np.max(np.sum(neighbors,axis=1)))))[:,:3] #ignore alpha channel
        for i in range(n_particles):
            x = pos[i,1]
            y = pos[i,0]
            pygame.draw.circle(screen,col[i],(int(((1-zoom)/2)*w + x*zoom*w/z),int(((1-zoom)/2)*h + zoom*y*h/z)),int(w/400))
        ...
        it += 1
        t0 = datetime.now()
        dphi = alpha + beta*rotationpart*np.sum(neighbors,axis=1)
